package com.hungry.customer.login.viewmodels;
...
public class LoginViewModel extends ViewModel {
    ...
    public void onClick(final View view) {
        ...
        if (isValidMobile(mobileNumber.getValue())) {
            ...
            if ((((Button)view).getText().toString() == view.getContext().getResources().getString(R.string.send_otp))) {
                        ...
                        public void onChanged(ResultVerifyOTP resultVerifyOTP) {
                            ...
                            if (resultVerifyOTP.status == 200) {
                                ...
                                if(resultVerifyOTP.result!=null && resultVerifyOTP.result.size()>=1){
                                    ...
                                    String androidId = Settings.Secure.getString(view.getContext().getContentResolver(),
                                            Settings.Secure.ANDROID_ID);

                                    token(resultVerifyOTP.result.get(0).getCM_MA_ID(),resultVerifyOTP.result.get(0).getCM_MOBILE(),androidId,null);
                ...
                if(otp!=null &&otp.getValue()!=null && !otp.getValue().isEmpty() && otp.getValue().length()==4) {
                    isloading.setValue(true);
    ...
    private void token(String clientid,String mobile,String android_id,String deviceid){
        ...
        FirebaseMessaging.getInstance().getToken()
                .addOnCompleteListener(task -> {
                    ...
                    LoginRepository loginRepository = new LoginRepository();
                    loginRepository.registerToken(clientid,mobile,token,"A",android_id,deviceid);
                    // Log and toast
